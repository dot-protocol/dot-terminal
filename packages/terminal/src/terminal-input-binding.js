...
export function bindTerminalInput({term, surface, controller, canDrop, notify, guardWindow = false, submit = text => controller.submit(text)}) {
  const stop = [];
  ...
  if(guardWindow) for (const type of ['dragover', 'drop']) on(window, type, event => { if (!event.defaultPrevented && meaningful(event)) event.preventDefault(); });
