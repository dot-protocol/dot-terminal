...
pub fn checked_service(input: &str, request: bool) -> Result<String, String> {
    use dot_terminal_protocol::{ServiceRequest, ServiceResponse, validate_service};
    if input.len() > MAX_FRAME {
        return Err("frame limit exceeded".into());
    }
    if request {
        let value: ServiceRequest = serde_json::from_str(input).map_err(|e| e.to_string())?;
        validate_service(&value).map_err(str::to_owned)?;
        serde_json::to_string(&value).map_err(|e| e.to_string())
    } else {
        let value: ServiceResponse = serde_json::from_str(input).map_err(|e| e.to_string())?;
        serde_json::to_string(&value).map_err(|e| e.to_string())
    }
}
...
#[cfg(target_os = "android")]
#[unsafe(no_mangle)]
pub extern "system" fn Java_world_dot_terminal_NativeBridge_checkService(
    mut env: jni::JNIEnv,
    _class: jni::objects::JClass,
    input: jni::objects::JString,
    request: jni::sys::jboolean,
