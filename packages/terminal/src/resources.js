...
export function mountDeviceResources(root,{request,device,sessionLabels={}}) {
 ...
 async function update(){
  ...
  try{
   ...
   const srows=(rows||[]).map((s,i)=>{const u=s.usage;const valid=u?.state==='sampled'&&sampleState({at:u.at,state:'ok'})==='live';return [sessionLabels[device.id+'/'+(s.id||s.session)]||'Terminal '+(i+1),valid?Number(u.cpu).toFixed(1)+'%':'—',valid?bytes(u.resident):'—'];});
